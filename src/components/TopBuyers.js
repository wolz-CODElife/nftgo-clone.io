...
import {TopBuyersHeading} from '../assets/data/TableHeading'
import { TopBuyersRows } from '../assets/data/TableRows';

function createData(whale, buyVol, bought, collection) {
  return { whale, buyVol, bought, collection };
}

const rows = TopBuyersRows.map(row => createData(...row));
export default function TopBuyers() {
  const [page, setPage] = useState(0);
  const [rowsPerPage, setRowsPerPage] = useState(10);
  const [pageNav, setPageNav] = useState([])
  const [timeRank, setTimeRank] = useState("1h")
  const [sort, setSort] = useState({ by: "buyVol", asc: "-1" })
          ...
          <TableBody>
            {(rowsPerPage > 0
              ...
              <TableCell align="left">
                ...
                <TableCell align="right">${row.buyVol || "--"}</TableCell>
            ...
            {emptyRows > 0 && (
              <TableRow style={{ height: 53 * emptyRows }}>
                <TableCell colSpan={TopBuyersHeading.length} />
              </TableRow>
            )}
          </TableBody>
        </Table>
