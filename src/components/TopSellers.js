...
import {timeRange} from '../assets/data/TableFilters'
import {TopSellersHeading} from '../assets/data/TableHeading'
import { TopSellersRows } from '../assets/data/TableRows';

function createData(whale, sellVol, sold, collection) {
  return { whale, sellVol, sold, collection };
}

const rows = TopSellersRows.map(row => createData(...row));
export default function TopSellers() {
  const [page, setPage] = useState(0);
  const [rowsPerPage, setRowsPerPage] = useState(10);
  const [pageNav, setPageNav] = useState([])
  const [timeRank, setTimeRank] = useState("1h")
  const [sort, setSort] = useState({ by: "sellVol", asc: "-1" })
    ...
    <TableContainer component={StyledTable}>
      ...
      <div className="table_responsive">
        <Table aria-label="caption table">
          <TableHead>
            <TableRow>
              {TopSellersHeading.map(heading => (
                <TableCell key={heading.title} align={heading.align || "left"} className={`${sort.by === heading.by ? sort.asc === "1" ? "asc up" : "asc down" : ""}`}>
          ...
          <TableBody>
            {(rowsPerPage > 0
              ...
              <TableCell align="left">
                <Tooltip arrow placement='top' title={row.whale}><span>🐋 {row.whale.split("").slice(0, 8).join("")}</span></Tooltip>
              </TableCell>
                <TableCell align="right">${row.sellVol || "--"}</TableCell>
            ...
            {emptyRows > 0 && (
              <TableRow style={{ height: 53 * emptyRows }}>
                <TableCell colSpan={TopSellersHeading.length} />
              </TableRow>
            )}
          </TableBody>
        </Table>
...
const PaginContainer = (props) => {
  const {page, count, rowsPerPage, pageNav, onPageChange} = props
  return (
    <Pagination >
